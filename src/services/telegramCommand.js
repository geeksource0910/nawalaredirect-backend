...
function extractDomain(text) {
  ...
  if (domainMatch) {
    ...
    const commonFalsePositives = ['gan', 'dong', 'sih', 'kok', 'aja', 'nya'];
...
/**
 * Core cek domain function - 3 source (IPOS + TrustPositif + Indiwtf)
 */
async function performDomainCheck(url, chatId, messageId, showLoading = true) {
  ...
  const [iposResult, tpBlocked, indiwtfResult] = await Promise.all([
    checkIPOS(url),
    checkDomainTrustPositif(url),
    checkDomainIndiwtfDirect(url),
  ]);
  ...
  // IPOS (real-time, primary)
  if (iposResult.error && iposResult.error !== 'NXDOMAIN') {
    message += `<b>IPOS (real-time):</b> ⚠️ ${iposResult.error}\n`;
  } else {
    message += `<b>IPOS (real-time):</b> ${iposResult.blocked ? '🚫 NAWALA' : '✅ AMAN'}\n`;
  }

  // TrustPositif (database)
  message += `<b>TrustPositif:</b> ${tpBlocked ? '🚫 NAWALA' : '✅ AMAN'}\n`;

  // Indiwtf (kalo available)
  ...
  // Final status: blocked kalo IPOS atau TrustPositif detect (Indiwtf optional)
  const finalBlocked = iposResult.blocked || tpBlocked ||
    (indiwtfResult.available && indiwtfResult.blocked);
...
async function handleHelpCommand(chatId, messageId) {
  ...
  message += `Cek 1 domain via IPOS + TrustPositif + Indiwtf\n\n`;
...
async function handleCasualChat(text, chatId, messageId) {
  ...
  if (/\b(bisa apa|fitur|kegunaan|fungsi|kerjaan|bantuin|bantu apa|what can you do|can you)\b/.test(lower)) {
    await sendReply(
      chatId,
      `Bisa lumayan bray 💪\n\n` +
      `🔍 <b>Cek domain</b> — pake 3 source (IPOS + TrustPositif + Indiwtf)\n` +
...
function isMentioningBot(message) {
  ...
  if (message.reply_to_message?.from?.username?.toLowerCase() === BOT_USERNAME.toLowerCase()) return true;
  if (message.entities) {
